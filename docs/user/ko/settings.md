...
### Hylink 주차 기능 — `leehyuk1108/carrotpilot:wip` 전용

2026-09-15부터 Hylink 자체의 11.5V 주차 기능 제한은 적용하지 않습니다. 전압 수치만으로
라이브·자동 사진·충격 감지·기록 업로드·Hylink 원격 터미널을 중지하지 않습니다.
시동·통신 상태·온도 보호는 유지하며, 콤마 자체 저전압 종료와 전원 설정은 변경하지 않습니다.
자체 종료는 기존 지연과 설정 조건을 따르므로 즉시 차단을 의미하지 않습니다.
이 변경은 앱 업데이트가 아니라 해당 `wip` 차량 소프트웨어 업데이트로 적용됩니다.
