...
## 후속 구현: IP:1108 + Hylink Dev 1.6.0

이 절이 현재 동작이다. 아래 **초기 d7a21a5d 기록**은 당시의 범위이며,
...
3. 휴대폰 브라우저에서 `http://콤마IP:1108`을 연다. 등록된 차량은 Wayon 키가 바로 나타난다.
   처음 연결하는 차량은 위치·상태·주행 기록 전송에 동의하고 원하는 주차 기능을 선택한다.
...
5. 같은 키 페이지에서 라이브/사진, 충격 감지, 원격 터미널을 개별 선택한다.
   콤마 `설정 → 기기 → Hylink 정보 전송 → 중지`로 전체 전송을 중지할 수 있다.

키는 위치·영상·선택한 원격 기능의 접근 권한이다. **같은 LAN의 다른 사람도 키를 볼 수 있는 방식**을
...
| 라이브, 라이브 사진/10·30초 클립 | 프로세스 종료 | 카메라 동의 후 사용 |
| IMU 충격 감지 | Hylink 감지 종료 | 동의 후 사용 |
| Hylink SSH 릴레이/셸 | 차단·종료 | 별도 동의 후 사용, 최대 5분 |
...
- 키 페이지의 설정 → 기기 config → manager → Cloud → 앱 기능 상태 표시.
- 충격 이벤트가 카메라 동의 후 발생하면 감지 후 사진을 `/api/impact-media`로 연결한다.
...
- 원격 SSH는 기존 Athena, `SshEnabled`, `GithubSshKeys`, 시스템 SSH 서비스를 건드리지 않는 별도 구현이다.
  ...
  SSH 명령 자체는 시스템을 변경할 수 있으므로 고급 기능으로 별도 동의한다. 원격 차량 조작 UI는 추가하지 않았다.
...
관련 Hylink/manager camera/Trailblazer 테스트 **141개**, 별도 manager 회귀 검사 **7개**가 통과했다.
실제 Worker 소스와의 오프라인 데이터 계약, APK 단위 테스트 **5개**/빌드/서명 일치,
브라우저 키 입력과 24개 화면 크기·텍스트 배치 조합을 검증했다.
로컬 HTTP 키 페이지의 최초 동의·키 표시·서버 종료 후 키 지우기도 브라우저에서 확인했다.
