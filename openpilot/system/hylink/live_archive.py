...
def post_capture_request(endpoint: str, payload: bytes, headers: dict) -> dict:
  if not endpoint.startswith(ENDPOINT + "/api/live-capture"):
    raise ValueError("Unexpected archive destination")
  for attempt in range(3):
    response = None
    try:
      response = requests.post(endpoint, data=payload, headers=headers, timeout=(5, 10), allow_redirects=False)
...
def upload_live_capture_chunked(config: dict, device_id: str, captured_at: str,
  ...
  if len(payload) > 32 * 1024 * 1024:
    raise ValueError("Capture exceeds 32 MiB budget")
