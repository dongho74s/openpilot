...
from openpilot.system.hylink.live_archive import build_dual_h264_archive, select_recent_frames, upload_live_capture
...
def test_live_capture_upload_retries_transient_http_error(monkeypatch):
  ...
  sleeps = []
  monkeypatch.setattr("openpilot.system.hylink.live_archive.requests.post", lambda *args, **kwargs: next(responses))
  monkeypatch.setattr("openpilot.system.hylink.live_archive.time.sleep", sleeps.append)

  result = upload_live_capture(
    {"endpoint": "https://wayon-cloud.hyuklee.workers.dev", "token": "token"},
    "device",
    "clip",
    "2026-07-21T01:00:00.000Z",
    "application/zip",
...
def test_large_clip_upload_uses_idempotent_chunks(monkeypatch):
  ...
  monkeypatch.setattr("openpilot.system.hylink.live_archive.LIVE_CAPTURE_CHUNK_BYTES", 4)
  monkeypatch.setattr("openpilot.system.hylink.live_archive.uuid.uuid4", lambda: type("Uuid", (), {"hex": "a" * 32})())
  monkeypatch.setattr("openpilot.system.hylink.live_archive.post_capture_request", fake_post)
